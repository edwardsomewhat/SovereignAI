...
| 05 Jun (cron #3)   | 55 | 9  | **Second-highest keeper count (16.4%).** 31 newly summarized + 24 orphaned. 9 kept, 46 deleted. Curated corpus 187→196. Default IP (100.84.92.74) worked without override — now 5+ consecutive runs reliable. ~23 min runtime. Grade-extraction bug still unpatched.
...
| 08 Jun (cron #2)   | 24 | 0  | 1 new summarized + 23 orphaned from prior interrupted runs. All 24 deleted. Every grade output was verbose reasoning — even sessions the model internally considered "A" or "B" ("SO THIS IS A B BECAUSE...", "A (KEEP): USER CORRECTION...") failed the exact-match check. First pipeline invocation timed out at 600s foreground (stdout buffered); re-ran capture/summarize/grade individually with `PYTHONUNBUFFERED=1`. Default IP (100.84.92.74) worked without override — 6+ consecutive runs reliable now. 0/24 keepers. Curated unchanged at 245.
